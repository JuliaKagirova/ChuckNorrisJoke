...
class MapScreen: UIViewController {
    ...
    var segmentedControl: UISegmentedControl = {
        var titles = ["MapScreen.segControll1".localized,
                      "MapScreen.segControll2".localized,
                      "MapScreen.segControll3".localized
        ]
    ...
    private func showAlert() {
        let alert = UIAlertController(title: "MapScreen.alert.title".localized,
                                      message: "MapScreen.alert.message".localized,
    ...
    private func checkAuthorization() {
        ...
        case .restricted:
            break
        case .denied:
            showAlertAction(title: "MapScreen.checkAuthorization.title".localized, message: nil, url: URL(string: UIApplication.openSettingsURLString ))
            break
        case .notDetermined:
            locationManager.requestWhenInUseAuthorization()
            break
    ...
    private func showAlertAction(title: String, message: String?, url: URL?) {
        ...
        let settingsAction  = UIAlertAction(title: "MapScreen.settingsAction".localized, style: .default) { (alert) in
            if let url = URL(string: "App-Prefs:root=LOCATION_SERVICES") {
                UIApplication.shared.open(url, options: [:], completionHandler: nil)
            }
        }
        let cancelAction = UIAlertAction(title: "MapScreen.cancelAction".localized, style: .destructive)
